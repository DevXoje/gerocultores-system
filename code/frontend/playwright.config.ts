...
export default defineConfig({
  ...
  // webServer is only needed for local development (BASE_URL = localhost).
  // When PLAYWRIGHT_TEST_BASE_URL is set (staging/preview URLs), the site is
  // already deployed — skip the local server entirely.
        ...
        webServer: {
          ...
          env: {
            ...
            VITE_FIREBASE_AUTH_DOMAIN:
              process.env.VITE_FIREBASE_AUTH_DOMAIN ?? 'dummy.firebaseapp.com',
            VITE_FIREBASE_PROJECT_ID: process.env.VITE_FIREBASE_PROJECT_ID ?? 'dummy-project',
            VITE_FIREBASE_STORAGE_BUCKET:
              process.env.VITE_FIREBASE_STORAGE_BUCKET ?? 'dummy.appspot.com',
