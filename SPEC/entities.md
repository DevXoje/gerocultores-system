...
- Los campos `diagnosticos`, `alergias`, `medicacion` y `preferencias` son datos de categoría especial (RGPD art. 9); solo accesibles por usuarios autenticados con rol `gerocultor` o `admin`.
...
- Solo el `Usuario` asignado o un `admin` puede modificar la tarea.
...
- Una incidencia de `severidad = 'critica'` dispara una `Notificacion` al administrador.
...
- **Reglas de Seguridad (Firestore Rules)**:
  ...
  - `residentes`: Lectura solo si el `request.auth.uid` está en el array de `gerocultoresAsignados` o el rol del usuario es `admin`.
...
- **Test Matrix de Reglas (Firestore)**:
  ...
  - *Escenario*: Administrador lee cualquier residente -> Permitido
