...
> El rol se mantiene en `role` para posibles ampliaciones futuras, pero el sistema asume `'gerocultor'` por defecto.
...
| `role` | `enum('gerocultor')` | sí | Rol del usuario (Firebase Custom Claim). Por defecto `'gerocultor'`. Reservado para futuras ampliaciones. |
...
**Relaciones**:
- Un `Usuario` de rol `gerocultor` tiene asignados 0..N `Residente` (vía `ResidenteAsignacion`).
...
- El sistema es **descentralizado**: no existe administrador global. Cada gerocultor es dueño de los recursos que crea.
- Un gerocultor **no puede ver ni modificar** los recursos (tareas, incidencias) de otro gerocultor.
...
- Los campos `diagnosticos`, `alergias`, `medicacion` y `preferencias` son datos de categoría especial (RGPD art. 9); solo accesibles por usuarios autenticados.
- Un gerocultor ve un residente si ha registrado al menos una tarea o incidencia sobre ese residente.
- Un gerocultor solo puede editar los campos de un residente que él mismo ha creado.
...
- Solo el `Usuario` asignado a la tarea puede modificarla (ownership).
...
- Una incidencia de `severidad = 'critica'` dispara una `Notificacion` al gerocultor que la registró.
...
- **Relaciones N:M (Asignaciones)**: La tabla `ResidenteAsignacion` se puede implementar como un array de `usuarioIds` dentro del documento del `residente` o una subcolección.
- **Modelo de seguridad**: Descentralizado. Cada gerocultor es dueño de sus recursos (tareas, incidencias). Los residentes son visibles para el gerocultor que los ha creado.
- **Reglas de Seguridad (Firestore Rules)**:
  ...
  - `tareas` e `incidencias`: Solo el gerocultor que las creó puede leer/escribir.
- **Test Matrix de Reglas (Firestore)**:
  - *Escenario*: Usuario no autenticado lee residentes -> Denegado
  - *Escenario*: Gerocultor lee residente que no ha creado -> Denegado
  - *Escenario*: Gerocultor lee residente que ha creado -> Permitido
  - *Escenario*: Gerocultor actualiza su propia tarea -> Permitido
  - *Escenario*: Gerocultor actualiza tarea de otro -> Denegado
  - *Escenario*: Gerocultor crea incidencia -> Permitido
<!-- fin delta -->
